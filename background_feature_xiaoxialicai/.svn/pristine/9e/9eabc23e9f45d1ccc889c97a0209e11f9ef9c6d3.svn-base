...
// 		}
    ...
    public function returnFundAction()
    {
        ...
        if ($frm->isThisFormSubmited()) {
           ...
           // if($realPayArr['realPayInterest']>$realPayInterest)return $this->returnError('利息支付超额！');
            ...
            $sn = !$plan['sn']?$this->_creatID():$plan['sn'];
    ...
    function subInterestFixAction(){
        ...
        $frm->addItem('_waresId_eq', form_def::factory('标的ID', '', form_def::text))
            ->addItem('_orderStatus_eq', form_def::factory('类型', '39', form_def::select, ([39=>'补发金额',10=>'补发计划'])))
            // ->addItem('_statusCode_eq', form_def::factory('订单状态', '', form_def::select,(\Prj\Consts\Wares::$enum+[''=>'全部'])))
            ->addItem('pageid', $pageid)
            ->addItem('pagesize', $pager->page_size);
        $frm->fillValues();
        $targetWaresId = $this->_request->get('targetWaresId');
        if($addInterestSub){
            $where['orderStatus'] = [\Prj\Consts\OrderStatus::done,\Prj\Consts\OrderStatus::going];
        }else{
            if ($frm->flgIsThisForm) { //submit
                $where = $frm->getWhere();
                $targetWaresId = $where['waresId='];
            } else {
                $where = [
        ...
        $waresArr = [];
        if($list){
            array_walk($list,function(&$v,$k)use(&$newList,$header,$addInterestSub,&$successNum,$sendInterestSub,&$waresArr,$targetWaresId){
                $ordersId = $v['ordersId'];
                   ...
                   // var_log($plan);
                    if($plan['interestSub']>0){
                        ...
                        if(empty($oldPlan['interestSub']) || $oldPlan['fix']){
                            if(!empty($oldPlan['realPayinterestSub']) && $oldPlan['isPay']==1 && empty($oldPlan['repay']))return true;
                        ...
                        if($addInterestSub || $sendInterestSub){
                            if($targetWaresId){
                                if($v['waresId']!=$targetWaresId){
                                    return true;
                                }else{
                                    error_log('targetWaresId >>>'.$targetWaresId);
                                }
                            }
                            if(!empty($oldPlan['realPayinterestSub']) || $oldPlan['repay']==8)return true;
                            ...
                            if($sendInterestSub){
                                ...
                                if($oldPlan['repay']!=8){
                                    //发送请求
                                    error_log('add waresId #'.$oldPlan['waresId']);
        ...
        if($sendInterestSub){
            ...
            if($waresArr){
                foreach($waresArr as $k=>$v){
                    $ware = \Prj\Data\Wares::getCopy($k);
                    $ware->load();
                    $returnPlan = $ware->getField('returnPlan');
                    $confirmSN = end($returnPlan['calendar'])['sn'];
                    if($newApi){
                        $data = [
                            0,$k,$confirmSN-0
                        ];
       ...
       // var_log($newList,'newList >>> ');
        ...
        $this->_view->assign('targetWaresId',$targetWaresId);
    }
