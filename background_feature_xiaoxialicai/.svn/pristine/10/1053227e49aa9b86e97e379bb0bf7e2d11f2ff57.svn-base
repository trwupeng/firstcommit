...
class LogerController extends \Prj\ServiceCtrl {
    ...
    protected function onStartupLogReceived($UserId, $extraData, $contractId = 0) {
        ...
        if ($this->loger->evt == 'start_up' || $this->loger->evt == 'wake_up') {
            ...
            $this->_view->assign('clientPatch', \Prj\Misc\ClientPatch::forStartup($this->loger->clientType, $this->loger->clientVer, $contractId));
        }
     ...
     * @input int firstStart 是否为首次登录,1为首次登录
     * @output {"code":200,"msg":""} 一般记录请求
     * @output {"code":200,"msg":"","contractId":0}  启动日志记录请求
     */
    public function applogAction() {
        //$isLogout = $this->_request->get('firstStart');
        //error_log('目前禁用掉启动时通过设置firstStart=1清除上一登入用户的能力');
//		if ($isLogout == 1) {
...
//		}
        ...
        $this->onStartupLogReceived($this->loger->userId, null, $this->loger->contractId);

        if ($this->loger->evt == 'start_up' || $this->loger->evt == 'wake_up') {
            if ($this->loger->contractId == \Prj\Data\Config::get('APP_PRO_CONTRACT_ID') && preg_match(\Prj\Data\Config::get('APP_PRO_VERSION'), $this->loger->clientVer)) {
                $this->_view->assign('interfaceType', 1);
            } else {
                $this->_view->assign('interfaceType', 0);
            }
        }
        $this->_view->assign('code', '200');
        $this->_view->assign('msg', '');
    ...
    public function holdAction() {
        ...
        if (empty($deviceType)) {
            return $this->returnError(\Prj\Lang\Broker::getMsg('loger.deviceType_error'));
        }
