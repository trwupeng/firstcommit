...
class Wechat {
    ...
    const MSG_TYPE_TRANSFER_CUSTOMER_SERVICE = 'transfer_customer_service';
    ...
    public function response($content, $type = self::MSG_TYPE_TEXT){
        ...
        $data = array(
            ...
            'CreateTime'   => \Sooh\Base\Time::getInstance()->timestamp(),
            'MsgType'      => $type,
        );
    ...
    private static function generate($data){
        ...
        $signature = self::sign(SoohBaseTime::getInstance()->timestamp(), $nonce, $encrypt);

        /* 加密消息基础数据 */
        $data = array(
            'Encrypt'      => $encrypt,
            'MsgSignature' => $signature,
            'TimeStamp'    => SoohBaseTime::getInstance()->timestamp(),
            'Nonce'        => $nonce,
        );
