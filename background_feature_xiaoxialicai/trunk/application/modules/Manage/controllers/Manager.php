    ...
    public function loginagainAction() {
        ...
        if (!empty($u) && !empty($p)) {
            ...
            if ($this->_chkPwd($acc, $p)) {
                ...
                $this->returnError(\Prj\Lang\Broker::getMsg('manager.admin_error_or_password_error_or_account_forbidden'));
            }
            ...
            if ($n == 0) {
                $acc->db()->addRecord($acc->tbname(), ['cameFrom' => 'local', 'loginName' => 'root', 'passwd' => '123456', 'rights' => '*.*']);
            }
        }
    ...
    protected function onInit_chkLogin() {
        try {
            ...
            if ($code === 300 || $code == 301) {
                if ($act === 'login' || $act === 'loginagain' || $act === 'index' || $act === 'welcome') {
