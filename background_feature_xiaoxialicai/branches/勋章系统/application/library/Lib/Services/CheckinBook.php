...
//								}
    ...
    public function doGetTodayStatus($withBonus, $userOrAccountId) {
        if ($this->rpc !== null) {
            return $this->rpc->initArgs(array('withBonus' => $withBonus,
                        'userOrAccountId' => $userOrAccountId))->send(__FUNCTION__);
        } else {
    ...
    protected function decode($var, $checkin = false) {
        ...
        if (\Sooh\Base\Tests\Bomb::blowup('CheckinBook_Decode_Return', false)) {
            $this->_view->assign('data', $this->r);
