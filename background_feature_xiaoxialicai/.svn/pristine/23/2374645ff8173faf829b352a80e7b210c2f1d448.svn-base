...
//	}
    ...
    public function counts($userId, $flgUsed) {
        $where = ['userId' => $userId];
        switch ($flgUsed) {
            case 1:$where['statusCode'] = \Prj\Consts\Voucher::status_used;
                break;
            case 0:$where['statusCode'] = \Prj\Consts\Voucher::status_unuse;
                break;
        }
...
//		error_log('Data\Voucher::newForUser');
        ...
        for ($retry = 0; $retry < 10; $retry++) {
            ...
            if (!$tmp->exists()) {
                ...
                if (empty($limit['limitsAmount']))
                    $tmp->setField('limitsAmount', 10000); //发红包默认满100可用
                $tmp->update();
...
//		$ret = $sys->db()->getRecord($sys->tbname(), '*', $map);
     ...
     * @param \Sooh\DB\Pager $pager   pager
     * @param string         $pagerId pagerId
     */
    ...
    public static function getTotalByUserId($userId) {
        ...
        $db = $voucher->db();
        $tb = $voucher->tbname();
        if ($userId == 90003837339748)
            var_log(\Sooh\Base\Time::getInstance()->ymdhis());
        $where = [
            'userId' => $userId,
            'voucherType' => \Prj\Consts\Voucher::type_real,
...
//	}
     ...
     * @param int $amount 红包金额 100=1元
     * @param int $voucherType 红包类型
     * @param int $expired 红包到期天数 2=2天
     * @param int $statusCode 红包状态
     * @param array $limit 红包限制 limit['limitsAmount']限制使用金额
     * @param int $pid 父红包id
     */
    public static function addVoucher($userId, $amount, $voucherType = \Prj\Consts\Voucher::type_real, $expired = 2, $voucherDesc = '', $statusCode = \Prj\Consts\Voucher::status_unuse, $limit = [], $pid = 0, $index = 1) {
        if ($index > 3) {   //只重复执行3次
            return FALSE;
        }
        $ordersId = self::getVoucherId($userId);
        $tmp = self::getCopy($ordersId);
        $tmp->load();
        if (!$tmp->exists()) {
            $expired = self::getDtExpired($expired);
            $tmp->setField('userId', $userId);
            $tmp->setField('voucherType', $voucherType);
            $tmp->setField('amount', $amount);
            $tmp->setField('timeCreate', date('YmdHis'));
            $tmp->setField('dtExpired', $expired);
            $tmp->setField('dtUsed', 0);
            $tmp->setField('orderId', 0);
            $tmp->setField('descCreate', \Prj\Misc\OrdersVar::$introForUser);
            ...
            $tmp->setField('pid', $pid);
            $tmp->setField('limitsAmount', (isset($limit['limitsAmount']) && !empty($limit['limitsAmount']) ? $limit['limitsAmount'] : 10000));
            $tmp->setField('createYmd', date('Ymd'));
            $tmp->setField('voucherDesc', $voucherDesc);
            if ($tmp->update()) {
                if ($voucherType == \Prj\Consts\Voucher::type_real) {
                    ...
                    if ($user->exists()) {
                        ...
                        $recentlyExpired = $user->getField('redPacketRecentlyExpired');
                        if ($expired > $nowTime && ($recentlyExpired == 0 || $expired < $recentlyExpired)) {
                            $user->setField('redPacketRecentlyExpired', $expired);
                        }
                        $user->setField('redPacket', $user->getField('redPacket') + $amount);       //更新红包余额
                        $user->update();
                    }
                }
                return $tmp;
            ...
            self::addVoucher($userId, $amount, $voucherType, $expired, $statusCode, $limit, $pid, ($index + 1));
        }
    }

    public static function getVoucherId($userId) {
        $ordersIdBase = substr($userId, -4);
        list($sec, $ms) = explode('.', microtime(true));
        return \Prj\Consts\OrderType::vouchers . $sec . substr($ms, 0, 3) . $ordersIdBase;
    }

    public static function getDtExpired($expired) {
        if (is_array($expired)) {
            $expired = date('YmdHis', $expired['end']);
        } else {
            $dt = \Sooh\Base\Time::getInstance();
