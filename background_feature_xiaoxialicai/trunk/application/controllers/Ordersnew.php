 ...
 * @author simon.wang
 */
class OrdersnewController extends \Prj\UserCtrl {
...
//新手标过滤
        ...
        if ($inputs['amount'] < $wares->getField('priceStart')) {
            return $this->returnError(\Prj\Lang\Broker::getMsg('orders.priceStart_out'));
        }
...
//满标改状态
        ...
        if ($user->getField('isSuperUser'))
            goto JUMPPAYGW;  //超级用户
        try {
            $data = [$orderId,$inputs['waresId'],$this->user->userId,$amount,$amountExt];
            $ret = \Lib\Services\PayGWCmd::sendToPayGWCmd('trade_investment',$data);
        ...
        if ($ret['code'] == 400 || empty($ret)) {
            ...
            if (empty($ret)) {
                $errorMsg = '网关未响应';
                return $this->returnError('网关未响应,请致电客服');
            } else {
                $errorMsg = $ret['msg'];
                return $this->returnError(($ret['msg'] ? $ret['msg'] : '系统错误') . ',请致电客服');
            }
        }
        JUMPPAYGW:
