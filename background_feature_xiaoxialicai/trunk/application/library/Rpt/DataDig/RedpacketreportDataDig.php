...
//        \Prj\Consts\Voucher::status_wait,
    ...
    public function getRedpacketCate() {
        ...
        $tmp = $this->dbMysql->getPair(\Rpt\Tbname::tb_vouchers_final, 'distinct(descCreate)', 'descCreate');
        return $tmp;
    }
    public function recordsByCategory($ymd){
        ...
        $produced = $this->dbMysql->getPair(\Rpt\Tbname::tb_vouchers_final, 'descCreate', 'sum(amount)/100 as n', $where, 'groupby descCreate');

        // 红包消耗
        $where = ['voucherType'=>\Prj\Consts\Voucher::type_real, 'ymdUsed'=>$ymd, 'statusCode'=>\Prj\Consts\Voucher::status_used];
        $used = $this->dbMysql->getPair(\Rpt\Tbname::tb_vouchers_final, 'descCreate', 'sum(amount)/100 as n', $where, 'groupby descCreate');

        // 红包失效
        $where = ['voucherType'=>\Prj\Consts\Voucher::type_real, 'statusCode'=>-99, 'dtExpired'=>$ymd];
        $overdue = $this->dbMysql->getPair(\Rpt\Tbname::tb_vouchers_final, 'descCreate', 'sum(amount)/100 as n', $where, 'groupby descCreate');
...
//红包产出排行','红包消耗排行','红包失效排行'
    public function rankRecords($ymd, $rankBase, $pkey=false) {
        ...
        switch($rankBase) {
            case 0:
                $where = ['ymdCreate'=>$ymd, 'voucherType'=>\Prj\Consts\Voucher::type_real, 'statusCode'=>$this->voucherAllStatus];
                break;
            case 1:
                $where = ['ymdUsed'=>$ymd, 'voucherType'=>\Prj\Consts\Voucher::type_real, 'statusCode'=>\Prj\Consts\Voucher::status_used];
    ...
    protected function getProduceByUser($ymd, $users) {
        $where =['userId'=>$users, 'ymdCreate'=>$ymd, 'voucherType'=>\Prj\Consts\Voucher::type_real, 'statusCode'=>$this->voucherAllStatus];
    ...
    protected function getUsedByUser($ymd, $users) {
        $where = ['userId'=>$users, 'ymdUsed'=>$ymd, 'voucherType'=>\Prj\Consts\Voucher::type_real, 'statusCode'=>1];
        return $this->dbMysql->getPair(\Rpt\Tbname::tb_vouchers_final, 'userId', 'sum(amount)/100 as n', $where, 'groupby userId rsort n');
    }
