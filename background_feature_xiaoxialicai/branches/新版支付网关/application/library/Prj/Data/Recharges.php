...
class Recharges extends \Sooh\DB\Base\KVObj{
    ...
    public static function getLastRedirectUrl($userId , $amountFlg){
        $tmp = self::getCopy($userId);
        $db = $tmp->db();
        $tb = $tmp->tbname();
        $where = ['userId'=>$userId,'amountFlg'=>$amountFlg];
        $record = $db->getRecord($tb,'*',$where,'rsort orderTime');
        $orderStatus = $amountFlg==\Prj\Consts\OrderType::recharges?\Prj\Consts\OrderStatus::waiting:\Prj\Consts\OrderStatus::created;
        if($record['orderStatus']!=$orderStatus || empty($record['redirectUrl'])){
            return [];
        }else{
            return ['amount'=>$record['amountAbs'],'redirectUrl'=>$record['redirectUrl'],'ordersId'=>$record['ordersId']];
        }
    }
