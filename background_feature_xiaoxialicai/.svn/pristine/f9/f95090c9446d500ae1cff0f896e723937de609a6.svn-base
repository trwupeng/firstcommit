...
//	protected $appId = 'wxaa49295fc15827a1';
...
//							$resources = (new \Prj\Oauth\Oauth($quickReg['code'], $quickReg['redirectUri']))->getResource();
							...
							if (\Sooh\Base\Tests\Bomb::blowup('Public_Login_Return_AccountId', false)) {
								$this->_view->assign('data', ['accountId' => $accountId]);//出现多个assign中的data字段重复，修改如下突出唯一识别
								$this->_view->assign('datareceivevoucher', ['accountId' => $accountId]);
							...
							if (!($user->exists())) {
								...
								if (\Sooh\Base\Tests\Bomb::blowup('Public_Login_Return_InvitationCode', false)) {
									...
									$this->_view->assign('datareceivevoucherto', ['invitationCode' => $invitationCode]);
									return $this->returnOK('bomb');
								}
...
//								}
								try {
									...
									if (empty($_finalItemsReg)) {
										...
										try {
											$_invireByUser = $user->getField('inviteByUser');
											if (empty($_invireByUser)) {
												\Prj\ReadConf::run(
													[
...
//		$openid = $this->authorizeForWechat();
		...
		if (\Lib\Misc\InputValidation::validateParams(['phone' => $phone], ['phone' => [\Lib\Misc\InputValidation::$define['phone'], '手机号不合法']]) === false) {
...
//			\Prj\Data\WechatBindPhone::bandUser($openid, $phone, $ret['accountId']);
	...
	public function recordRegEvtAction()
	{
		...
		if (strtoupper($_SERVER['REQUEST_METHOD']) == 'GET') {
			...
			$requestParams = array_map('htmlspecialchars', $requestParams);

//			$a             = isset($requestParams['a']) ? $requestParams['a'] : '';//手机输入框
//			$b             = isset($requestParams['b']) ? $requestParams['b'] : '';//密码输入框
//			$c             = isset($requestParams['c']) ? $requestParams['c'] : '';//短信验证码输入框
//			$reged         = isset($requestParams['reged']) ? $requestParams['reged'] : '';//_md_lastRegSuc_，一般在点击注册后触发， 初始值为0
//			$sendcode      = isset($requestParams['sendcode']) ? $requestParams['sendcode'] : '';//_md_lastSendCode_，一般在点击获取验证码后触发， 初始值为0
//			$lt            = isset($requestParams['lt']) ? $requestParams['lt'] : '';//_md_lastForm，最后获取焦点的输入框（）
//			$source        = isset($requestParams['source']) ? $requestParams['source'] : '';//_source，渠道号
//			$channel       = isset($requestParams['channel']) ? $requestParams['channel'] : '';//前端类型

			//将信息记录到数据库中
//			\Prj\Data\Regevt::addOne($requestParams);
			...
			$_ret = [
				...
				'his'       => \Sooh\Base\Time::getInstance()->his,
				'a'         => isset($requestParams['a']) ? $requestParams['a'] : '',
				'b'         => isset($requestParams['b']) ? $requestParams['b'] : '',
				'c'         => isset($requestParams['c']) ? $requestParams['c'] : '',
				'reged'     => isset($requestParams['reged']) ? $requestParams['reged'] : '',
				'sendcode'  => isset($requestParams['sendcode']) ? $requestParams['sendcode'] : '',
				'lt'        => isset($requestParams['lt']) ? $requestParams['lt'] : '',
				'source'    => isset($requestParams['source']) ? $requestParams['source'] : '',
				'channel'   => isset($requestParams['channel']) ? $requestParams['channel'] : '',
				'_post'     => json_encode($_POST),
				'_get'      => json_encode($_GET),
				'_cookie'   => json_encode($_COOKIE),
			];
			error_log('======Reg Pages Report Data======&'.http_build_query($_ret));
		}
		exit(0);
	}
