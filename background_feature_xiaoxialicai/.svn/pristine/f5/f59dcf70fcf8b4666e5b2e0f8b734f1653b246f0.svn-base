...
class ContractsController extends \Prj\ManagerCtrl
{
    ...
    protected $plans;
    protected $pageSizeEnum = array(30, 60, 100);
...
// var_log($where , 'where>>>>>>>>>>>>');
    ...
    public function addnewAction() {
        if (empty($this->copartners)) {
           ...
           $this->returnError(\Prj\Lang\Broker::getMsg('contracts.copartners_unfound'));
            return;
        }
        ...
        $formEdit->addItem('contractId', form_def::factory('协议Id', '', form_def::text)/*->verifyInteger(1, 100000000000000000, 999999999999999999)*/)
                ...
                ->addItem('displayRule', form_def::factory('展示数据规则', '0', form_def::select)->initMore(new options_def(\Prj\Data\Contract::$displayRule)))
        ...
        if ($formEdit->flgIsThisForm){
            try{
                ...
                if(strlen($contractId)!=18){
                    ...
                    return $this->returnError($op.\Prj\Lang\Broker::getMsg('contracts.contractId_rules'));
                }
                // 字段检查先放这里了。
                if($fields['displayPercent'] >100 || $fields['hidePercent'] <0 ) {
                   // return $this->returnError('隐藏比例请输入0-100的数字');
                    return $this->returnError(\Prj\Lang\Broker::getMsg('contracts.displayPercent_error'));
                }
                $obj = contract_kobj::getCopy(array('contractId'=>$contractId));
                $pkey = $obj->load();
                if ($pkey !==null){
                   // $this->returnError($op.'失败：协议号已经存在');
                    return $this->returnError($op.\Prj\Lang\Broker::getMsg('contracts.contractId_already_existing'));
                }
                if (substr($contractId, 0, 4) != $this->copartnerIds[$fields['copartnerAbs']]){
                   // $this->returnError('协议号和渠道号不匹配，应该以渠道号开头');
                    return $this->returnError(\Prj\Lang\Broker::getMsg('contracts.contractId_unmatch_copartners'));
...
// var_log($fields, 'fields>>>>>');
                ...
                if ($pkey===null){
                   // $this->returnError('不能更新，无相关记录');
                    return $this->returnError(\Prj\Lang\Broker::getMsg('contracts.upd_not_record'));                    
                }
                foreach ($fields as $k => $v){
                    $obj->setField($k, $v);
                }
            ...
            if ($pkey===null){
               // $this->returnError('记录找不到');
                $this->returnError(\Prj\Lang\Broker::getMsg('contracts.record_unfound'));
            }else {
