 ...
 * OK
 *
 * php /var/www/licai_php/run/crond.php "__=crond/rungrab&task=RptDaily.EDAccounts&ymdh=20160128"
 * 日报账户统计
...
class EDAccounts extends \Sooh\Base\Crond\Task{
	...
	protected function oneday($ymd)
	{
		...
		$newReg->reset();
		$db = \Sooh\DB\Broker::getInstance(\Rpt\Tbname::db_p2prpt);//,'flagUser!'=>1
		$rs = $db->getRecords(\Rpt\Tbname::tb_user_final,
				'flagUser,clientType, copartnerId, count(*) as total ',
				['ymdReg['=>$ymd],
				'group clientType group copartnerId group flagUser');
...
//		var_log($rs);
		...
		//,'flagUser!'=>1
		$rs = $db->getRecords(\Rpt\Tbname::tb_user_final, 'flagUser,clientType, copartnerId, count(*) as total ',['ymdReg'=>$ymd],'group clientType group copartnerId group flagUser');
