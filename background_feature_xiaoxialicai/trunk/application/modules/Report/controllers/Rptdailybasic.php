	...
	if($acl->hasOperation()){
		...
		$lst['p_buyinfo']=[
				'fmt'=>['平台购买情况','总计{BuyNUsrDay}人(增长{BuyNUsrDay_UP}%，首投用户数占{newBuyerRate}%）<br>'
						. '购买{BuyNAmountDay}元(增长{BuyNAmountDay_UP}%)'],
				'extraRate'=>['{newBuyerRate}'=>['{BuyNUsrNew}','{BuyNUsrDay}']],
		];
//				'p_rebuyinfo'=>[
//		'fmt'=>['平台复投情况','todo'],
//		'extraRate'=>[],
...
//			旧的格式
//			$lst2['p_realnameauth'] =['当日实名认证数'=>'{RealnameAuth}'];
//			$lst2['p_realnameauthnewreg'] = ['当日注册实名认证数'=>'{RealnameAuthNewReg}'];
...
//			新的格式
			...
			$lst2['p_bindok'] = [
				'当日绑卡'=>[
					'绑卡成功总数'=>'{BindOk}',
					'注册并绑卡成功人数'=>'{RegAndBindOk}',
					'绑卡失败总数'=>'{BindFailed}',
					'注册并绑卡失败人数'=>'{RegAndBindFailed}',
				],
			];
...
//					'新增理财人数'=>'{BuyNUsrNew}',
					'注册并理财人数'=>'{BuyNUsrNew0}',
					'老用户首购理财人数'=>'{BuyNUsrNew1}',
					'老投资理财人数'=>'{BuyNUsrOlder}',
					'理财金额'=>'{BuyNAmountDay}',
//					'首购理财金额'=>'{BuyNAmountNew}',
					'注册并理财金额'=>'{BuyNAmountNew0}',
					'老用户首购理财金额'=>'{BuyNAmountNew1}',
					'老投资理财金额'=>'{BuyNAmountOlder}',
				],
			];
...
//			$this->fillRs($rs, 'BuyUsrNew0', $ymd, $tomorrow, $db,['flgext02!'=>1]);
			...
			$this->fillRs($rs, 'BuyNUsrNew', $ymd, $tomorrow, $db,['flgext02!'=>1]);
			$this->fillRs($rs, 'BuyNUsrNew0', $ymd, $tomorrow, $db,['flgext02!'=>1]);
			$this->fillRs($rs, 'BuyNUsrNew1', $ymd, $tomorrow, $db,['flgext02!'=>1]);
			$this->fillRs($rs, 'BuyNUsrOlder', $ymd, $tomorrow, $db,['flgext02!'=>1]);
			$this->fillRs($rs, 'BuyNAmountNew1', $ymd, $tomorrow, $db,['flgext02!'=>1]);
			$this->fillRs($rs, 'BuyNAmountOlder', $ymd, $tomorrow, $db,['flgext02!'=>1]);

//					'理财人数'=>'{BuyNUsrDay}',
//					'注册并理财人数'=>'{BuyNUsrNew0}',
//					'老用户首购理财人数'=>'{BuyNUsrNew1}',
//					'老投资理财金额'=>'{BuyNUsrOlder}',
//					'理财金额'=>'{BuyNAmountDay}',
//					'注册并理财金额'=>'{BuyNAmountNew0}',
//					'老用户首购理财金额'=>'{BuyNAmountNew1}',
//					'老投资理财金额'=>'{BuyNUsrOlder}',
...
//			$rs[$ymd]['{YebAmountIncrease}'] = prjlib_evtdaily::getCopy('YebAmountIncrease')->numOfAct($db, $ymd);
			...
			$rs[$ymd]['{RealnameAuthNewReg}'] = prjlib_evtdaily::getCopy('RealnameAuthNewReg')->numOfAct($db, $ymd);
//			$rs[$ymd]['Stock'] = prjlib_evtdaily::getCopy('Stock')->numOfAct($db, $ymd);
//
//
//			$rs[$ymd]['NewChargeNum'] = prjlib_evtdaily::getCopy('NewChargeNum')->numOfAct($db, $ymd);
