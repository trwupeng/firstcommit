...
namespace PrjCronds;
/**
 * php /var/www/licai_php/run/crond.php "__=crond/rungrab&task=Standalone.CrondTagUserForDisplay&ymdh=20150819"
...
error_log('###################################执行日期'.$ymd.'###################################');
        foreach(\Prj\Data\Contract::$displayRule as $rule => $name) {
            switch($rule) {

                case \Prj\Data\Contract::unlimitrule:  // 没有任何规则限制的用户
                    $this->unlimitRule($ymd, $rule);
                    break;
                case \Prj\Data\Contract::regrule:      // 注册规则的用户
                    $this->regRule($ymd, $rule);
                    break;
                case \Prj\Data\Contract::bindrule:     // 绑卡规则的用户
                    $this->bindRule($ymd, $rule);
                    break;
                case \Prj\Data\Contract::buyrule:      // 购买规则的用户
    ...
    protected function unlimitRule($ymd, $rule) {
        ...
        $count = $this->db_rpt->getRecordCount(\Rpt\Tbname::tb_user_final, ['ymdReg'=>$ymd, 'flagDisplay'=>1]);
//var_log($count, '当日打标记数目:');
        if($count>0) {
            error_log('已经给无扣量规则的当日注册用户打过标记了');
            return;
        }
...
//error_log('给无扣量规则的渠道打标记：'.\Sooh\DB\Broker::lastCmd());
    }
    ...
    protected function buyRule($ymd, $rule) {
        ...
        foreach($reg_contract as $contractId) {
            ...
            $buy_user = $this->db_rpt->getCol(\Rpt\Tbname::tb_orders_final, 'distinct(userId)',
                ['userId'=>$users, 'ymd'=>$ymd,'orderStatus!'=>$failed_order_status]);
    ...
    protected function getLimitContracts ($rule) {
        $records =\Prj\Data\Contract::loopFindRecordsByFields(['displayRule'=>$rule], null, 'contractId,displayPercent', getRecords);
        $tmp = [];
        if(!empty($records)) {
            foreach($records as $r) {
                $tmp[$r['contractId']] = $r['displayPercent'];
            }
        }
        return $tmp;
    }
