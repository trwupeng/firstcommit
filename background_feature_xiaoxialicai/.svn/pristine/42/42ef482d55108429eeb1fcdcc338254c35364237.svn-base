    ...
    */
    ...
    public function bindcardAction()
    {
        ...
        if (!empty($inputs['cmd'])) {
            ...
            //检测银行维护
            if(\Prj\Misc\BankStop::checkStop($inputs['bankId']))return $this->returnError(\Prj\Misc\BankStop::getNotice($inputs['bankId']));
    ...
    public function rechergeAction(){
        ...
        $myCard = current($myCards);
        //银行维护检测
        if(\Prj\Misc\BankStop::checkStop($myCard['bankId']))return $this->returnError(\Prj\Misc\BankStop::getNotice($myCard['bankId']));
