...
class Oauth {
	...
	public function __construct($code = '', $redirectUri = '', $mode = 'standard') {
		...
		var_log($this->_get('accessToken', false, true), 'Obj init: get->accessToken');
		var_log($this->_get('refreshToken', false, true), 'Obj init: get->refreshToken');
...
//			$this->loger->sarg2 = json_encode($args);
	...
	protected function _set($name, $value, $allowNulls = false, $setExtra = true, $extraArgs = []) {
		...
		$this->$name = $value;
		var_log($this->_get($name, false, true), 'storage something ret:' . $name);
	}
...
//			\Sooh\Base\Log\Data::getInstance('c')->sarg1 = $url;
			...
			if ($check && $response['code'] == '60017' && $this->retryNum < 3) {
