...
class IceLoger {
	...
	*/	
		$data = json_encode( array( 'cmd' => 'services/loger/writeSyn', 'logdata' => $logData, 'request' => $_SOOH_REQUEST_INFO ) );
		$ini = \Sooh\Base\Ini::getInstance();
		$localObj = \Sooh\DB\Nosql\Nosql::getInstance( $ini->getItem('dbConf')['redis'] );
		$localObj->list_pushBack( \Sooh\Base\Tools::getRedisListKey(), $data );
