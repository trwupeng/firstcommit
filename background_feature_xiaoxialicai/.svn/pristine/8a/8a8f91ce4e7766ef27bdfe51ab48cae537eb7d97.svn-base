...
class AssetController extends \Prj\ManagerCtrl {
    ...
    public function indexAction($type = '') {
        ...
        if (!$isDownloadEXCEL) {
            $rs = Asset::pager($pager, $where, 'sort status rsort createTime');
        } else {
        ...
        foreach ($rs as $k => $v) {
            ...
            $tempArr['remainDays'] = strtotime($tempArr['endYmd']) - strtotime(date('Ymd000000')) > 0 ? (strtotime($tempArr['endYmd']) - strtotime(date('Ymd000000'))) / 86400 : 0;
    ...
    public function editAction() {
        ...
        if ($frm->flgIsThisForm) { //submit
            ...
            if ($type == 'check') {
                ...
                $asset->load();
                if ($asset->getField('status') != \Prj\Consts\Asset::status_new)
                    return $this->returnError('该资产已经审核过了');
    ...
    public function deleteAction() {
        ...
        $asset->load();
        if (!$asset->exists())
            return $this->returnError('资产不存在');
        if ($asset->getField('status') == \Prj\Consts\Asset::status_ok)
            return $this->returnError('上线的资产无法删除');
    ...
    public function rejectAction() {
        $where = \Prj\Misc\View::decodePkey($this->_request->get('_pkey_val_'));
        $count = \Prj\Data\Wares::loopGetRecordsCount(['assetId' => $where['assetId']]);
        if ($count > 0)
            return $this->returnError('存在关联标的,禁止驳回');
    ...
    public function detailAction() {
        ...
        if ($pkey = null) {
            ...
            $editorClass = '\Prj\WaresTpl\\' . $asset->getField('viewTPL') . '\Editor';
            $editor = $editorClass::getCopy($asset->getField('introDisplay'));
            $this->_view->assign('editor', $editor);
            ...
            foreach ($ks as $k) {
                if ($asset->exists($k)) {
                    ...
                    if ($k == 'startYmd' || $k == 'endYmd') {
                        ...
                        } elseif ($viewTPL == 'Std04') {
                            $formView->item($k)->value = '房贷审核';
                        } elseif ($viewTPL == 'Std05') {
                            $formView->item($k)->value = '车贷审核';
                        }
                    } else {
            ...
            $formView->item('amount')->value = $asset->getField('amount') / 100;
            $formView->item('remain')->value = $asset->getField('remain') / 100;
        }
    }
...
//       $ware=$wares->loopFindRecords(['assetId'=>$where['assetId']]);
        ...
        $frm->addItem('_waresname_lk', form_def::factory('标的名称', '', form_def::text))
                ->addItem('_assetId_eq', form_def::factory('资产Id', $where['assetId'], form_def::text))
                ->addItem('pageid', $pageid)
        ...
        $pager->total =  $ware->db()->getRecordCount($ware->tbname(),$where);
    ...
    public function newUploadAction() {
        ...
        if (empty($_FILES['file']))
            return $this->returnError(\Prj\Lang\Broker::getMsg('wares.img_missing'));
    ...
    public function doSplitAction() {
        ...
        var_log($count, '已经拆分了>>>>>>>>');
        if (!empty($count))
            return $this->returnError('已经拆分成' . $count . '个标,请勿重复操作！');
        $wares = $this->_split($data);
