 ...
 */
class Sqlsrvazure implements \Sooh\DB\Interfaces\All
{
 ...
 */		
	...
	protected function connect()
	{
		...
		$servierId = explode('.',$conf['host']);
		$servierId = array_shift($servierId);
		$this->_connection = sqlsrv_connect("tcp:{$conf['host']}, 1433",array("UID"=>$conf['user']."@".$servierId,"PWD"=>$conf['pass'],"Database"=>$defaultDB) );
...
//		return $rs;
	...
	public function getRecords($obj, $fields, $where=null, $orderby=null,$pagesize=null,$rsfrom=0)
	{
		...
		while (null!==($r = sqlsrv_fetch_array($rs0,SQLSRV_FETCH_ASSOC))){
			...
			$rs[]=$r;
			//if(sizeof($rs)>40000){
			//	error_log(">>big-rs-found".$_REQUEST['cmd']);
			//}
		}
		sqlsrv_free_stmt($rs0);
		return $rs;
