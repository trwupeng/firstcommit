...
class PassportController extends \Prj\UserCtrl
{
    ...
    public function logoutAction()
    {
        ...
        \Sooh\Base\Session\Data::getInstance()->set('snMark', null);
    ...
    {
        //短信通道60s限制
        $_sess    = \Sooh\Base\Session\Data::getInstance();
        $_nowTime = \Sooh\Base\Time::getInstance()->timestamp();
        $_sessTime = $_sess->get('sendSmsCodeForUpdPwdLastTime');
        if ($_sessTime <= $_nowTime && $_nowTime - $_sessTime < 60) {
            return $this->returnError(\Prj\Lang\Broker::getMsg('passport.smscode_has_been_sent'));
        }
        ...
        try {
            ...
            $_sess->set('sendSmsCodeForUpdPwdLastTime', $_nowTime);
    ...
    public function sendSmscodeForQuickLoginAction()
    {
        ...
        $_sess    = \Sooh\Base\Session\Data::getInstance();
        $_nowTime = \Sooh\Base\Time::getInstance()->timestamp();
        $_sessTime = $_sess->get('sendSmsCodeForQuickLogin');
        if ($_sessTime <= $_nowTime && $_nowTime - $_sessTime < 60) {
            return $this->returnError(\Prj\Lang\Broker::getMsg('passport.smscode_has_been_sent'));
        }
        ...
        try {
            ...
            try {
                (new \Prj\Oauth\Oauth('', '', 'nonStandardMode'))->invokeMode($tmp_oauthMap);
            } catch (\Exception $e) {
                return $this->returnError(\Prj\Lang\Broker::getMsg('passport.phone_number_is_not_registered'));
            }
            ...
            $_sess->set('sendSmsCodeForQuickLogin', $_nowTime);
     ...
     * 登录
     * 返回值说明
     * hasBeenInvited:是否被邀请过（1已被邀请，0未被邀请）
    ...
    public function loginAction()
    {
        ...
        try {
            ...
            if (!empty($curClientType)) {
                if (\Lib\Misc\InputValidation::validateParams(['clientType' => $curClientType], [
                        'clientType' => [
                            inputValidation::$define['clientType'],
                            \Prj\Lang\Broker::getMsg('passport.clientType_is_not_valid')
                        ]
                    ]) == false
                ) {
                ...
                if (empty($dbFirstLoginApp) && ($curClientType == \Prj\Consts\ClientType::android || $curClientType == \Prj\Consts\ClientType::appstore)) {
                    ...
                    try {
                        ...
                        if (empty($finalItems)) {
                            //TODO send firstLogin redPacket fail
                        } else {
                            $this->loger->sarg3 = json_encode([
                                [
                        ...
                        error_log('send NewFirstLoginAppRedPacket faild:' . $e->getMessage());
                    }
                }
            }
            ...
            if (isset($_finalItemsReg)) {
                $this->_view->assign('redPacket', [
                    'type'      => 'RedPacketForRegister',
                    'amount'    => $_finalItemsReg[0][1],
                    'dtExpired' => $_finalItemsReg[0][2]
                ]);
            }
     ...
     * @input integer keepLogin 是否登录（1登录， 0不登录）
    ...
    public function quickRegAction()
    {
        ...
        $_keepLogin = $this->_request->get('keepLogin', 0);
        ...
        $rules = [
            ...
            'smscode'    => [
                inputValidation::$define['smscode'],
                \Prj\Lang\Broker::getMsg('passport.smscode_is_not_valid')
            ],
            'clientType' => [
                inputValidation::$define['clientType'],
                \Prj\Lang\Broker::getMsg('passport.clientType_is_not_valid')
            ...
            'contractId' => [
                inputValidation::$define['contractId'],
                \Prj\Lang\Broker::getMsg('passport.contractId_is_not_valid')
            ],
        ];
        ...
        try {
            ...
            if (!($user->exists())) {
                ...
                try {
                    ...
                    $_finalItemsReg = $itemGiverReg->add('NewRegisterRedPacket', 1)->give();
                    if (empty($_finalItemsReg)) {
                        //TODO deal send register redPacket fail
                    } else {
                        $this->loger->sarg1 = json_encode([
                            [
                                'event'        => 'red_loging_packet',
    ...
    public function quickLoginAction()
    {
        ...
        $rules = [
            ...
            'smscode'      => [
                \Lib\Misc\InputValidation::$define['smscode'],
                \Prj\Lang\Broker::getMsg('passport.smscode_is_not_valid')
            ],
            'clientId'     => [
                \Lib\Misc\InputValidation::$define['clientId'],
                \Prj\Lang\Broker::getMsg('passport.clientId_is_not_valid')
            ],
            'clientSecret' => [
                \Lib\Misc\InputValidation::$define['clientSecret'],
                \Prj\Lang\Broker::getMsg('passport.clientSecret_is_not_valid')
            ],
        ];
            ...
            if (!empty($curClientType)) {
                ...
                if (empty($dbFirstLoginApp) && ($curClientType == \Prj\Consts\ClientType::android || $curClientType == \Prj\Consts\ClientType::appstore)) {
                    ...
                    try {
                        ...
                        $finalItems = $itemGiver->add('NewFirstLoginAppRedPacket', 1)->give();
                        if (empty($finalItems)) {
                            //TODO deal send firstlogin app redpacket fail
                        } else {
                            $this->loger->sarg3 = json_encode([
                                [
                                    'event'        => 'first_login_packet',
    ...
    public function resetPwdAction()
    {
        ...
        $rules = [
            ...
            'deviceId'     => [
                '#^[0-9a-zA-Z_\-:]{10,50}$#',
                \Prj\Lang\Broker::getMsg('passport.deviceId_is_not_valid')
            ],
            'clientId'     => [
        ...
        try {
            ...
            $this->loger->sarg1 = json_encode(['all', $ret['accountId'], null, $pushContent]);
            ...
            if (!($user->exists())) {
                ...
                try {
                    ...
                    \Prj\Data\User::getCopy($inviteByUser)->update();
                    if ($weekActiveBonus) {
                        $this->loger->sarg3 = json_encode(['all', $inviteByUser, null, json_encode($weekActiveBonus)]);
                    }
                } catch (Exception $e) {
