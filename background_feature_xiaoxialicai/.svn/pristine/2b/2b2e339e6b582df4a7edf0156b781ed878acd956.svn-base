...
class OrdersController extends \Prj\UserCtrl
{
    ...
    protected function setInviteCodeAction($code){
        ...
        if(empty($code))return $this->returnError(\Prj\Lang\Broker::getMsg('user.setinvite_emptycode'));
     ...
     * @errors {"code":400,"msg":"error_paypwd"} 支付密码错误
...
//	return $this->returnError('购买系统升级中，请稍后再试。');
        ...
        if(empty($inputs['cmd']))return $this->returnError(\Prj\Lang\Broker::getMsg('orders.no_cmd'));
        ...
        if($inputs['cmd']==='buy')
        {
            ...
            if (!empty($inputs['paypwd'])) { //检查支付密码
                ...
                if ($_lyq_ret !== true) {
                    ...
                    \Sooh\Base\Ini::getInstance()->viewRenderType('json');
					$this->_view->assign('errorMsg', $_lyq_ret['msg']);
					$this->_view->assign('errorCount', $_lyq_ret['errorCount']);
    ...
    protected function _redPacket($waresId,$amount)
    {
        ...
        foreach($rs as $k=>$v)
        {
            if($total+$v['amount']>$amount)break;
    ...
    protected function add_real($inputs)
    {
        ...
        if ($_SERVER['REMOTE_ADDR']) $this->loger->ip = $_SERVER['REMOTE_ADDR'];
        ...
        $wares->load();
        if (!$wares->exists()) return $this->returnError(\Prj\Lang\Broker::getMsg('orders.no_wares')); //标的不存在 'no_wares'
        //新手标过滤
        ...
        */
        if ($wares->getField('statusCode') == \Prj\Consts\Wares::status_flow) return $this->returnError(\Prj\Lang\Broker::getMsg('orders.ware_flow')); //不可购买的标的状态 'not_open'
        if ($wares->getField('statusCode') != \Prj\Consts\Wares::status_open) return $this->returnError(\Prj\Lang\Broker::getMsg('orders.not_open')); //不可购买的标的状态 'not_open'
        ...
        if ($walletRemain < $amount) {
            error_log('error:walletRemain is not enough');
        ...
        if (!empty($inputs['voucherId']) || !empty($redArr)) {
            ...
            if(!empty($redArr))$inputs['voucherId'] = empty($inputs['voucherId'])?$redArr:array_merge($inputs['voucherId'],$redArr);
            ...
            foreach ($inputs['voucherId'] as $k => $v) {
                ...
                if (empty($voucherArr[$v])) {
                    ...
                    if (!class_exists($vrTPL)) return $this->returnError(\Prj\Lang\Broker::getMsg('orders.voucherTPL_invalid')); //'voucherTPL_invalid'
                    $vrArr[$v] = $vrTPL::createRule($v);//tgh 实例化券规则
                    if (!empty($vrArr[$v]->error)) return $this->returnError(\Prj\Lang\Broker::getMsg('orders.voucherTPL_invalid')); //'voucherRule_invalid'
                    //非红包券数量
                    if ($voucherArr[$v]->getField('voucherType') != \Prj\Consts\Voucher::type_real) {
                        $notRedCount++;
                    }
                    //额外加成计算
                    if ($vrArr[$v]->isReturn) $amountExt += $vrArr[$v]->returnAmount;
                    if ($vrArr[$v]->isFake) $amountFake += $vrArr[$v]->fakeAmount;
                    if ($vrArr[$v]->isYield) $addYield += $vrArr[$v]->addYield;
                }
            }
            if ($notRedCount > 1) return $this->returnError(\Prj\Lang\Broker::getMsg('orders.voucher_out')); //'voucher_out'
        ...
        if($inputs['cmd'] == 'buypaypwd')
        {
            ...
            if($amount!=$invest->getField('amount') || $inputs['waresId']!=$invest->getField('waresId')
            || $this->user->userId!=$invest->getField('userId') )
            {
        ...
        //锁定用户
        if (!$this->user->lock(\Prj\Misc\OrdersVar::$introForCoder . " orderId:" . $orderId . " withAmount:" . $amount . ' withVoucher:' . $inputs['voucherId'])) {
        ...
        if(!$user->getField('isSuperUser'))$wares->setField('realRaise', $wares->getField('realRaise') + $amount); //真实募集总额
...
/////////////////////////////////////////////////////////TODO
            ...
            try {
                ...
                $evt->result($this->user->userId, $inputs['waresId'], $orderId, $amount,
                    1, $inputs['clientType'], $ymdFirstBought ? 0 : 1);
       ...
       if($user->getField('isSuperUser'))goto JUMPPAYGW;  //超级用户
        try{
            $ret = $sys->addOrder($orderId,$inputs['waresId'],$this->user->userId,$amount,$amountExt,$amountFake,$invest->getField('orderTime'),$invest->getField('orderStatus'));
        ...
        //网关返回错误
        if(empty($ret)){
            error_log('trace_orders:'.$orderId."[网关错误]by user:" . $this->user->userId . " orderId:" . $orderId . " withAmount:" . $amount . ' withVoucher:' . $inputs['voucherId'],'error>>>');
        ...
        if (!$ymdFirstBought) { //首购松泉 //debug  //
            ...
            if (!empty($newVoucherId)) {
                try {
					...
					error_log('trace_orders:'.$orderId.'#msg for voucher for first time send failed:'.$e->getMessage());
                }
            }
        ...
        $wares->unlock();
		error_log('trace_orders:'.$orderId.'# wares finaly unlock');
        $this->user->unlock();
		error_log('trace_orders:'.$orderId.'# user finaly unlock');
       ...
       //推送
	    ...
	    try {
			...
			error_log('trace_orders:'.$orderId.'# jpush sent');
	    } catch (\ErrorException $e) {
			error_log('trace_orders:'.$orderId.'# jpush send failed');
	    }
        //活跃值-子红包次数
        ...
        if(empty($parUserId)){
            ...
            try{
                ...
                if(!empty($weekActiveBonus)){
                    \Lib\Services\Push::getInstance()->push( 'all', $parUserId, null,
    ...
    protected function add_rollback($wares, $tally, $invest , $user = null)
    {
        ...
        if ($invest) {
            try {
                ...
                $invest->updStatus(\Prj\Consts\OrderStatus::abandon)->update();
            } catch (\ErrorException $e) {
    ...
    public function myRedPacketAction()
    {
        ...
        if (empty($waresId)) return $this->returnError(\Prj\Lang\Broker::getMsg('system.arg_error'));
        $wares = \Prj\Data\Wares::getCopy($waresId);
        $wares->load();
        if (!$wares->exists()) return $this->returnError(\Prj\Lang\Broker::getMsg('orders.no_wares'));
        \Sooh\Base\Ini::getInstance()->viewRenderType('json');
