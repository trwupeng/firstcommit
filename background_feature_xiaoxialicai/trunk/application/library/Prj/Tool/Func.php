...
class Func {
    ...
    public static function getUrlFromSinaHtml($content){
        $preg = "/(action=['\"].*?['\"])|(value=['\"].*?['\"])/";
        preg_match_all($preg,$content,$matches);
        if(empty($matches[0][0]) || empty($matches[0][1]))return '';
        $url = preg_replace("/(action=['\"])|['\"]/",'',$matches[0][0]);
        $ft = preg_replace("/(value=['\"])|['\"]/",'',$matches[0][1]);
        return $url.'?ft='.$ft;
    }
}
