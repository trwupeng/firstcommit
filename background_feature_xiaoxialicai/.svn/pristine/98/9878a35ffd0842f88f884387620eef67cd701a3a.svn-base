...
class RedPacketForShare extends RedPacket {
	...
	public function give($userId) {
		...
		if ($ret !== '') {
			...
			try {
				\Prj\ReadConf::run(
					['event' => 'red_share_packet', 'num_packet' => '1', 'num_money' => $this->getAmountLast()[0] / 100, 'num_deadline' => 30],
...
//		$cur    = $user->getField('redPacket');
		$userId = $user->userId;
		for ($i = 0; $i < $num; $i++) {
			list($expire, $amount, $type) = [['end' => \Sooh\Base\Time::getInstance()->timestamp(5)], $this->getAmountByInvestment($investment, $investmentExpires), \Prj\Consts\Voucher::type_share];
...
//		$_ret = $this->give_prepare_child($voucherId, $uniqueId, $userId);
...
//				$voucherId = $this->voucherIds;
				...
				try {
					\Prj\ReadConf::run(
						['event' => 'red_share_packet', 'num_packet' => '1', 'num_money' => $amount, 'num_deadline' => 30],
						['phone' => $user->getField('phone'), 'userId' => $userId]
					);
				} catch(\Exception $e) {
					var_log($e->getMessage(), 'Send RedPacketForShare Message Error');
				}
				return ['type' => 'investChild', 'amount' => $amount, 'id' => $childVoucherId, 'pid' => $pid, 'dtExpired' => $dtExpired];
			}
		} else {
	...
	{
		//此人是否领取过
//		$_ret = \Prj\Data\Vouchers::findOne(['uniqueId' => $uniqueId, 'pid' => $pid, 'voucherType' => \Prj\Consts\Voucher::type_real]);
		...
		//是否超过每日限制
//		$_totalNum = \Prj\Data\Vouchers::loopGetRecordsCount(['uniqueId' => $uniqueId, 'codeCreate' => 'investChild', 'createYmd' => \Sooh\Base\Time::getInstance()->YmdFull]);
...
//	protected function give_prepare_child_to_voucher($user, $amount, $vouchersId, $uniqueId, $pid, $num = 1)
	{
		...
		for ($i = 0; $i < $num; $i++) {
			if ($this->shareFromInvestment) {
				list($expire, $amount, $type) = [['end' =>\Sooh\Base\Time::getInstance()->timestamp(self::childDayExpired)], $amount, \Prj\Consts\Voucher::type_real];
			} else {
...
//			$tmp = \Prj\Data\Vouchers::newForUserForChild($userId, $type, $amount, $vouchersId, $pid, $uniqueId, $expire);
		...
		if ($this->setStatus != \Prj\Consts\Voucher::status_wait) {
			//give child voucher and update user redPacket
