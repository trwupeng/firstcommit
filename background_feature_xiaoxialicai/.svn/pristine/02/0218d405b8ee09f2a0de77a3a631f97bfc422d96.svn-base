	...
	public function tttAction()
{
	...
	public function swapUserAction()
	{
		...
		try {
			//交换tb_loginname
//			$dbPhone2->setField('cameFrom', 'tmp_phone');
//			$dbPhone2->update();
//			$dbPhone1->setField('loginName', $phone2);
//			$dbPhone1->update();
//			$dbPhone2->setField('loginName', $phone1);
//			$dbPhone2->setField('cameFrom', 'phone');
//			$dbPhone2->update();
			...
			if (preg_match('/^[a-zA-Z]{6}\d{4}$/', $_user_nickname1) >= 1) {
...
//			$dbUser1->setField('salt', $_user_tradeSalt2);
			...
			if (preg_match('/^[a-zA-Z]{6}\d{4}$/', $_user_nickname2) >= 1) {
...
//			$dbUser2->setField('salt', $_user_tradeSalt1);
			...
			$dbUser2->update();
			var_log(\Sooh\DB\Broker::lastCmd(false), 'swap tb_user');
