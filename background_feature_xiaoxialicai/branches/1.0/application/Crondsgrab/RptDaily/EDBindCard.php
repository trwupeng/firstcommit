...
class EDBindCard extends \Sooh\Base\Crond\Task{
    ...
    protected function oneday($ymd) {
        ...
        $where = 'createYmd='.$ymd.' and statusCode in (16,4) ';
        ...
        $where = 'createYmd='.$ymd.' and statusCode =0 and resultMsg!=\'\' '
            .'and tb_bankcard_final.userId not in (select tb_bankcard_final.userId from db_p2prpt.tb_bankcard_final where statusCode in(4,16) and createYmd='.$ymd.') ';
        $result = $this->getResult($this->sqlBase($where));
        if (!empty($result)) {
            foreach($result as $r) {
        ...
        $where = 'createYmd='.$ymd.' and statusCode in (16,4) and ymdReg='.$ymd;
