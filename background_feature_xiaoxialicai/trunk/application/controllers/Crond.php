...
//                        $ret = \Prj\Items\RedPacketForFirstBuyForInvite::sendRebateRedPacket($v,$ymd);
    ...
    public function autoConfirmAction(){
        ...
        error_log("####[warning]自动还款定时任务开始###".$ymd);
        if(date('H')<17 || date('H')>20){
            error_log('###17点~20点之间才会执行###');
            goto over;
        }
        $lazy = 1; //延缓执行
        ...
        if($list){
            foreach($list as $v){
                ...
                error_log('###autoConfirm#'.$v['waresId'].'还款计划>>>'.json_encode($plan));
                if(empty($plan)){
                ...
                error_log('###autoConfirm#autoConfirm#'.$v['waresName'].'#'.$v['waresId'].'#该笔处理状态#'.$plan['status']);
                ...
                try{
                    if($newApi){
                        ...
                        $ret = \Prj\Wares\Wares::doConfirm($plan['sn']-0,$plan['interest']+$plan['amount'],0,$wares->getField('managementConfirm'),$plan['planDateYmd'],$plan['ahead'],0,$wares);
                    }else{
                        error_log('autoConfirm#执行旧的支付网关...');
                        $ret = \Prj\Wares\Wares::getCopy()->confirm($wares,$plan);
                    }
                }catch (\ErrorException $e){
                    error_log('[warning]'.$v['waresName'].'#'.$v['waresId'].'#'.$e->getMessage()."\n\n");
                    continue;
                }
