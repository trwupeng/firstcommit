...
class DevController extends \Lib\Dev\ApidocCtrl
{
	...
	{
		$cok = $_COOKIE['SoohSessId'];
		setcookie('SoohSessId',$cok);
		$this->returnOK('done');
	}
...
//			$notification = ['default' => 'notifiTest2'];
			...
			\Lib\Services\Push::getInstance()->push($platform ? : 'android', $userId, $notification ? : null, $msg ? : 'JPush Test!~');
			return $this->returnOK('success');
		} catch (Exception $e) {
			return $this->returnError($e->getMessage(), $e->getCode());
		}
    ...
    public function showUrlAction()
    {
        ...
        {
            $arrPage[$k] = 'http://'.$_SERVER['HTTP_HOST'].$v;
...
//		$itemGiver->give();
    ...
    public function tgh_testAction(){
        \Sooh\Base\Ini::getInstance()->viewRenderType('json');
        \Prj\Items\Rebate::rebateResult('145949253387754997',41,'888865724142','success');
    }
    ...
    public function tgh_addNewWareAction(){
        ...
        $fields['waresName'] = '系统标-'.date('md').'-'.rand(1000,9999);
        ...
        $fields['autoReturnFund'] = 1;
        $fields['autoConfirm'] = 1;
        ...
        $fields['borrowerId'] = '46061253494017';
	...
	public function rateTestAction()
	{
		...
		for($i = 1; $i < 8; $i++) {
			for($j = 0; $j < 1000; $j++) {
				$checkin[$i][] = $this->getAmountByCheckinAction($i, $checkinRule, $maxAmount);
			}
		}
		...
		foreach ($checkin as $k => $v) {
			if ($k < 7) {
				foreach ($v as $_k => $_v) {
					if ($_v >= 16 && $_v < 24) {
						if (isset($ret['check'][$k]['16_24'])) {
							$ret['check'][$k]['16_24']++;
						} else {
							$ret['check'][$k]['16_24'] = 1;
						}
					} elseif ($_v >= 24 && $_v < 33) {
						if (isset($ret['check'][$k]['24_33'])) {
							$ret['check'][$k]['24_33']++;
						} else {
							$ret['check'][$k]['24_33'] = 1;
						}
					} elseif ($_v >= 33 && $_v < 41) {
						if (isset($ret['check'][$k]['33_41'])) {
							$ret['check'][$k]['33_41']++;
						} else {
							$ret['check'][$k]['33_41'] = 1;
						}
					} elseif ($_v >= 41 && $_v < 50) {
						if (isset($ret['check'][$k]['41_50'])) {
							$ret['check'][$k]['41_50']++;
						} else {
							$ret['check'][$k]['41_50'] = 1;
						}
					} elseif ($_v >= 50 && $_v < 58) {
						if (isset($ret['check'][$k]['50_58'])) {
							$ret['check'][$k]['50_58']++;
						} else {
							$ret['check'][$k]['50_58'] = 1;
						}
					} else {
						if (isset($ret['check'][$k]['58_67'])) {
							$ret['check'][$k]['58_67']++;
						} else {
							$ret['check'][$k]['58_67'] = 1;
						}
				...
				$ret['check'][7]['sum'] = array_sum($v);
				$ret['check'][7]['avg'] = array_sum($v) / 1000;
			}
		}

		foreach ($ret['check'] as $k => &$v) {
			if ($k < 7) {
				foreach ($v as $_k => $_v) {
					$v[$_k] = '出现' . $_v . '次，占比：' . $_v / 10 . '%';
				}
			}
		}
    ...
    function tgh_withdrawResultAction(){
        ...
        $ret = \Lib\Services\PayGW::getInstance()->withdrawResult($ordersId,$withdraw->getField('amountAbs'),'success');
        $this->_view->assign('ret',$ret);
    }

	public function lyqTestAction()
	{
//		\Lib\Services\SMS::getInstance()->sendCode('18616700069', '', 'hahahahahahah');
	}
