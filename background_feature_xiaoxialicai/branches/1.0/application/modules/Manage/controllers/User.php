...
class UserController extends \Prj\ManagerCtrl
{
    ...
    public function indexAction()
    {
        ...
        if (!empty($this->borrower)) $fieldsMapArr['isBorrower'] = ['是否借款人', '30'];
        ...
        $where = array_merge($where, $this->borrower);
...
// 			$user=new User();
// 			$records = $user->db()->getRecords($user->tbname(), implode(',', array_keys($fieldsMapArr)), $where, 'rsort ymdReg');
// 		} else {
...
// 		}
        ...
        if ($isDownloadEXCEL) {
            ...
            if ($pager->pageid() == 1) {
                ...
                $ret = \Prj\Data\User::loopGetRecordsPage(['ymdReg' => 'rsort'], ['where' => $where], $pager);
            } else {
        ...
        $header = [];
        unset($fieldsMapArr['hisReg']);
        unset($fieldsMapArr['points']);
        ...
        foreach ($records as $v) {
            foreach ($fieldsMapArr as $kk => $vv) {
                if ($kk == 'phone') {
				    ...
				    $temp[$kk] = $v[$kk].sprintf("%06d",$v['hisReg']);
//var_log($temp[$kk], 'ymdReg>>>>>');				    
				}elseif($kk=='ymdFirstBuy'){
				    $temp[$kk]=\Prj\Misc\View::fmtYmd($v[$kk]);
				}elseif($kk=='ymdLastBuy'){
				    $temp[$kk]=\Prj\Misc\View::fmtYmd($v[$kk]);
				}
				else {
                    $temp[$kk] = $v[$kk];
            ...
            $temp['ymdReg'] = \Prj\Misc\View::fmtYmd($temp['ymdReg'], 'time');
            $temp['dtLast'] = \Prj\Misc\View::fmtYmd($temp['dtLast'], 'time');
    ...
    public function rechargeAction()
    {
        ...
        if ($frm->flgIsThisForm) {
            $where = $frm->getWhere();
            if (!empty($where['orderTime]'])) $where['orderTime]'] = date('YmdHis', strtotime($where['orderTime]']));
            if (!empty($where['orderTime['])) $where['orderTime['] = date('Ymd' . '235959', strtotime($where['orderTime[']));
        } else {
            $where = array();
        }
        //拉取记录
        ...
        foreach ($rs as $k => $v) {
            ...
            foreach ($fieldsMapArr as $kk => $vv) {
                $tempArr[$kk] = $v[$kk];
                if (empty($tempArr[$kk])) $tempArr[$kk] = '';
            }
    ...
    public function withdrawAction()
    {
        ...
        if (!empty($where['withdrawYmd]'])) $where['withdrawYmd]'] = date('Ymd', strtotime($where['withdrawYmd]']));
        if (!empty($where['withdrawYmd['])) $where['withdrawYmd['] = date('Ymd', strtotime($where['withdrawYmd[']));
        var_log($where, '查询条件>>>>>>>>>>>>>>>>');
        //$rs = \Prj\Data\Recharges::loopAll($where);
        $pager->total = \Prj\Data\Recharges::loopGetRecordsCount($where);
...
//             }
    ...
    public function sendWithdrawingAction()
    {
        ...
        if (empty($orderIds)) return $this->returnError(\Prj\Lang\Broker::getMsg('user.no_orderId'));
        ...
        if (!empty($orderIds)) {
            foreach ($orderIds as $k => $v) {
                ...
                $withdraw[$k]->load();
                if (!$withdraw[$k]->exists()) return $this->returnError($v . '不存在于资料库中');
        ...
        if (empty($ret['payCorp'])) return $this->returnError('no_payCorp');
        $error = '';
            ...
            try {
                ...
                if ($frm->type() == \Sooh\Base\Form\Broker::type_c) //add
                {
                    ...
                    */
                    ...
                    $recharge->load();
                    if(!$recharge->exists()){return $this->returnError('不存在的流水号');}
                    if($recharge->getField('orderStatus')==\Prj\Consts\OrderStatus::done)return $this->returnError('重复的操作');
                    ...
                    while($retry>=0 && !$user->lock(date('H:i:s').'#retryRecharge#ordersId:'.$fields['ordersId'])){
                        if($retry==0)return $this->returnError('系统正忙,请稍后重试');
                        $user->reload();
                        $retry--;
                    }
                    $user->reload();
        ...
        if ($frm->flgIsThisForm)
        {
            ...
            try {
                ...
                if ($frm->type() == \Sooh\Base\Form\Broker::type_c) //add
                {
                    ...
                    if(!$trans){return $this->returnError('系统错误:生成记录失败');}
                    //调用网关
