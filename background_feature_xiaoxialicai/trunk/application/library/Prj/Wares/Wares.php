            ...
            foreach($investList as $v){
                ...
                if(empty($plan)){
                    ...
                    if($plan['isPay']==1 || $plan['status']==\Prj\Consts\PayGW::accept ){ //
                        ...
                        //8月27日0：00后上架的标的关闭贴息
                        if($ware && $ware->getField('timeStartReal')>'20160827000000'){
                            error_log('returnFundAll#ordersId:'.$ordersId.'#贴息关闭,停止发放>>>');
                            $realPayArr['interestSub'] = 0;
                            $rp->updatePlanByMonth('interestSub',$realPayArr['interestSub'],$plan['planDateYmd']);
                        }
                        $num++;
                        $tmp = [
                            'cmd'=>'returnFund',
