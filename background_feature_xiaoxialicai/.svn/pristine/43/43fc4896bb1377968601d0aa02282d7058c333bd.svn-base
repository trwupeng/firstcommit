...
class Rebate {
    ...
    public static function openRebate($orderId){
        $mark = '#openRebate#orderId:'.$orderId.'#';
        $tmp = \Prj\Data\Rebate::getCopy('');
        $rs = $tmp::loopFindRecords(['investId'=>$orderId,'statusCode'=>[\Prj\Consts\OrderStatus::created,\Prj\Consts\OrderStatus::payed,\Prj\Consts\OrderStatus::going,\Prj\Consts\OrderStatus::failed,\Prj\Consts\OrderStatus::waitingGW],'amount>'=>0])[0];
        error_log($mark.'select date result:'.(is_array($rs)?json_encode($rs):$rs));
        if(empty($rs)){
            return [];
        }else{
            ...
            if(!$rebate->exists()){
                ...
                if(in_array($rebate->getField('statusCode'),[\Prj\Consts\OrderStatus::waitingGW,\Prj\Consts\OrderStatus::done])){
                    throw new \ErrorException('rebate have pay...');
                }
                $sn = substr($rebateId,0,14).rand(1000,9999);
                $sn = $rebate->getField('sn')?$rebate->getField('sn'):$sn;
                var_log($sn,'sn>>>');
                $mark.=('sn:'.$sn.'#');
                ...
                try{
                    ...
                    error_log($mark.'访问网关失败#'.$e->getMessage());
                    if($e->getCode()==400){
                ...
                try{
                    error_log($mark.'rebate update begin ...');
                    $rebate->update();
                }catch (\ErrorException $e){
                    usleep(500000);
                    try{
                        error_log($mark.'rebate update retry ...');
                        $rebate->update();
                    }catch (\ErrorException $e){
                        error_log($mark.'rebate update failed ...');
                    }
                }
            }
            return $ret;
        ...
        if(empty($rs)){
            ...
            if(!$rebate->exists()){
                ...
                if($status=='failed'){
                    ...
                    $user->load();
                    if(!$user->exists()){
                        self::rollback(null,null,null,'用户不存在userId:'.$userId);
                    }else{
                        //todo 返利加钱
                        ...
                        if(empty($tally)){
                            ...
                            //更新流水
                            try{
                                error_log($mark.'tally update begin ...');
                                $tally->update();
                            }catch (\ErrorException $e){
                            ...
                            try{
                                ...
                                error_log($mark.'user update begin ...');
                                $user->update();
                            }catch (\ErrorException $e){
