...
class BidRemind extends \Sooh\Base\Crond\Task
{
                            ...
                            while ($_retry > 0) {
                                try {
                                    if ($dbUser->exists()) {
                                        ...
                                        error_log('bidRemind not found user:' . $user['userId']);
                                        break;
                                    }
                                } catch (\Exception $e) {
                                    error_log('tye update user error, e:' . $e->getMessage());
                                    $dbUser->reload();
                                    $_retry--;
                                }
                            }
    ...
    {
        if (\Prj\Message\Message::checkPush($userId, 20)) {
            try {
                $message = \Prj\Message\Message::load(
                    [
