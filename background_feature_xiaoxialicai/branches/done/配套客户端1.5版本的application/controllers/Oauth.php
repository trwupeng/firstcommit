...
//		$_start_time = microtime(true);
		...
		try {
			...
			//调用同盾接口
			$account->checkSecureForTongDun($accountInfo['accountId'], $params);
...
//		$_start_time = microtime(true);
		...
		try {
			...
			if (isset($params['tongDunStr'])) {
				//调用同盾接口
				\Lib\Services\Account::getInstance(\Prj\BaseCtrl::getRpcDefault('Account'))->checkSecureForTongDun($ret['accountId'], $params);
			}
        ...
        try {
            ...
            if ($dbLogin->exists()) {
	            ...
	            try {
		            ...
		            \Prj\ReadConf::run(
			            ['event' => 'reg_name', 'brand' => \Prj\Message\Message::MSG_BRAND, 'num_num' => $smsCode, 'num_time' => \Prj\Message\Message::MSG_NUM_TIME_15],
