...
class MonthreportController extends \Prj\ManagerCtrl {
    ...
    public function indexAction () {
        ...
        $formEdit->addItem('_yearFrom_eq', form_def::factory('从', date('Y'), form_def::select)->initMore(new \Sooh\Base\Form\Options($this->year)))
                 ->addItem('_monthFrom_eq', form_def::factory('年', '1', form_def::select)->initMore(new \Sooh\Base\Form\Options($this->month)))
                 ->addItem('_yearTo_eq', form_def::factory('月，到', date('Y'), form_def::select)->initMore(new \Sooh\Base\Form\Options($this->year)))
                 ->addItem('_monthTo_eq', form_def::factory('年', date('n'), form_def::select)->initMore(new \Sooh\Base\Form\Options($this->month)))
...
// var_log($where, __FILE__.'>>>where>>>>');
        ...
        if ($where['ymd['] < $where['ymd]']) {
           // $this->returnError('起始时间应该小于或等于结束时间');
            $this->returnError(\Prj\Lang\Broker::getMsg('monthreport.strattime_lessthan_endtime'));
        }
        if ($saveAsExcel) {
            // 导出复选框选中的记录， 未做 
