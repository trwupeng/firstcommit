        ...
        if ($formEdit->flgIsThisForm){ // 添加
            try {
                ...
                if(ctype_lower((string)substr($fields['copartnerAbs'], 0, 1)) || ctype_upper((string)substr($fields['copartnerAbs'], 0, 1))){
                    ...
                    $this->returnError($op.\Prj\Lang\Broker::getMsg('失败：渠道简称必须是英文'));
                    return;
                }
                ...
                if ($pkey!==null){
                   // $this->returnError($op.'失败：相关记录已经存在');
                    $this->returnError($op.\Prj\Lang\Broker::getMsg('copartners.record_already_existing'));
                    return;
                }
        ...
        if ($formEdit->flgIsThisForm){
            try {
                ...
                if(ctype_lower((string)substr($fields['copartnerAbs'], 0, 1)) || ctype_upper((string)substr($fields['copartnerAbs'], 0, 1))){
                    ...
                    //$this->returnError($op.'失败：渠道简称第一位必须是字母');
                    $this->returnError($op.\Prj\Lang\Broker::getMsg('copartners.copartnerAbs_frist_ letter'));
                    return;
                }
                ...
                if ($pkey===null) {
                    ...
                    $this->returnError(\Prj\Lang\Broker::getMsg('copartners.upd_not_record'));
                    return;
                }
                foreach ($fields as $k => $v){
                    $obj->setField($k, $v);
...
//                 $this->returnError($op.'失败：'.$e->getMessage());
            ...
            if ($pkey===null){
               // $this->returnError('记录找不到');
                $this->returnError(\Prj\Lang\Broker::getMsg('copartners.record_unfound'));
            }else {
