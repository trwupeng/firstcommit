...
class User extends \Sooh\DB\Base\KVObj {
     ...
     * 获得投资客数量
     */
    public static function getAllBuyUser() {
        $index = isset($GLOBALS['CONF']['dbByObj']['user'][0]) ? $GLOBALS['CONF']['dbByObj']['user'][0] : 2;
        $count = 0;
        $db = \Sooh\DB\Broker::getInstance(\Rpt\Tbname::db_p2p);
        for ($i = 0; $i < $index; $i++) {
            $sql = "SELECT COUNT(*) AS c FROM tb_user_{$i} WHERE ymdFirstBuy != 0";
            $result = $db->execCustom(['sql' => $sql]);
            $result = $db->fetchAssocThenFree($result);
            $count += $result[0]['c'];
        }
        return $count;
    }
