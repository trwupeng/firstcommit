...
class Rebate extends \Sooh\DB\Base\KVObj{
    ...
    public static function cancelByInvestId($investId){
        $rs = self::loopFindRecords(['investId'=>$investId,'statusCode!'=>\Prj\Consts\OrderStatus::done , 'statusCode!'=>\Prj\Consts\OrderStatus::abandon]);
        $record = current($rs);
        if(empty($record))return null;
        $rebate = self::getCopy($record['rebateId']);
        $rebate->load();
        if(!$rebate->exists())return 'rebate_miss';
        try{
            $rebate->setField('statusCode',\Prj\Consts\OrderStatus::abandon);
            $rebate->update();
            error_log('rebate->cancelByInvestId#investId:'.$investId.'# update success...');
        ...
        if($user->exists()){
            ...
            while(true){
                $retry++;
                $newRebating = $user->getField('rebating') - $record['amount'];
                $user->setField('rebating',$newRebating?$newRebating:0);
                try{
                    $user->update();
                    error_log('rebate->cancelByInvestId#investId:'.$investId.'# update user success...');
                    break;
                }catch (\ErrorException $e){
                    if($retry>=2){
                        $rebate->setField('statusCode',\Prj\Consts\OrderStatus::created);
                        $rebate->update();
                        return $e->getMessage();
                    }
                    sleep(1);
                    error_log('rebate->cancelByInvestId#investId:'.$investId.'# update user retry...');
                    $user->reload();
                }
            }
        }else{
