...
namespace PrjCronds;
/**
 * php /var/www/licai_php/run/crond.php "__=crond/rungrab&task=RptDaily.EDStockingByClient&ymdh=20160126"
 *
 * Created by PhpStorm.
 * User: li.lianqi
 * Date: 2016/1/26 0026
...
//error_log('withdraw total add:'.$withdrawDay->totalAdd);
        ...
        $where = 'ymd='.$ymd.' and amount<0 and orderStatus in (3,39) ';
        $result = $this->getResult($this->sqlBase($where));
        if (!empty($result)) {
            foreach($result as $r) {
...
//error_log('apply withdraw add:'.$applyWithdrawDay->totalAdd);
    ...
    protected function sqlBase ($where) {
        return 'select tb_user_final.clientType,sum(tb_recharges_final.amount)/100 as n,tb_user_final.copartnerId '
            .'from db_p2prpt.tb_recharges_final '
            .'left join db_p2prpt.tb_user_final on tb_recharges_final.userid=tb_user_final.userId '
            .'where '.$where
