...
class WechatAuth {
	...
	const ACCESS_TOKEN_EXPIRES   = 6000;
	const JSAPI_TICKET_EXPIRES   = 6000;
	const errGetAccessTokenError = '获取accessToken失败';
	const errGetJsapiTicketError = '获取jsapi_ticket失败';
	 ...
	 * @throws \Exception
	 */
	public function getAccessToken($type = 'client', $code = null, $refresh = false) {
		if (!$refresh) {
			//获取本地access_token
			...
			if ($accessToken) {
				...
				$accessToken = \Prj\Data\Wechat::get('accessToken');
				if (!empty($accessToken) && is_array($accessToken) && $accessToken['expiresIn'] > \Sooh\Base\Time::getInstance()->timestamp()) {
		...
		if (is_array($token)) {
			if (isset($token['errcode'])) {
				...
				try {
					\Prj\Data\Wechat::set('accessToken', [
						'accessToken' => $token['access_token'],
						'expiresIn'   => \Sooh\Base\Time::getInstance()->timestamp() + self::ACCESS_TOKEN_EXPIRES
					]);
				} catch (\Exception $e) {
					throw new \Exception(self::errGetAccessTokenError);
				}
	...
	public function getJsapiTicket($refresh = false) {
		if (!$refresh) {
			if ($this->jsapiTicket) {
				...
				$jsapiTicket = \Prj\Data\Wechat::get('jsapiTicket');
				if (!empty($jsapiTicket) && is_array($jsapiTicket) && $jsapiTicket['expiresIn'] > \Sooh\Base\Time::getInstance()->timestamp()) {
					$this->jsapiTicket = $jsapiTicket['jsapiTicket'];
					return $jsapiTicket['jsapiTicket'];
				}
			}
		}

		$url = "{$this->oauthApiURL}/ticket/getticket";
		$ret = json_decode(self::http($url, ['type' => 'jsapi', 'access_token' => $this->getAccessToken()]), true);
		if (is_array($ret)) {
			if ($ret['errcode'] == 0) {
				$this->jsapiTicket = $ret['ticket'];
				try {
					\Prj\Data\Wechat::set('jsapiTicket', [
						'jsapiTicket' => $ret['ticket'],
						'expiresIn'   => \Sooh\Base\Time::getInstance()->timestamp() + self::JSAPI_TICKET_EXPIRES
					]);
				} catch (\Exception $e) {
					throw new \Sooh\Base\ErrException(self::errGetJsapiTicketError);
				}
			} else {
				throw new \Sooh\Base\ErrException($ret['errmsg']);
			}
		} else {
			throw new \Sooh\Base\ErrException(self::errGetJsapiTicketError);
		}
	...
	/**
	 * 网页授权，根据code获取用户信息并保存下来
	 * @param string $code code
	 * @return bool|string $openid
	 * @throws \ErrorException
	 * @throws \Exception
	 */
	public function saveUserinfoFromCode($code)
	{
		$params = [
			'appid' => $this->appId,
		    'secret' => $this->appSecret,
		...
		if ($ret && !isset($ret['errcode'])) {
			...
			var_log($accessToken, '===========web authorize access token==========');
			...
			$_ret = self::http($_url, $_params);
			$_ret = json_decode($_ret, true);
			if ($_ret && !isset($_ret['errcode'])) {
				$dbWechatUserinfo = WechatUserinfo::getCopy($openid);
				$dbWechatUserinfo->load();
				$dbWechatUserinfo->setField('nickname', base64_encode($_ret['nickname']));
				$dbWechatUserinfo->setField('sex', $_ret['sex']);
				$dbWechatUserinfo->setField('province', $_ret['province']);
				$dbWechatUserinfo->setField('city', $_ret['city']);
				$dbWechatUserinfo->setField('country', $_ret['country']);
				$dbWechatUserinfo->setField('headimgurl', $_ret['headimgurl']);
				$dbWechatUserinfo->setField('privilege', $_ret['privilege']);
				$dbWechatUserinfo->setField('unionid', $_ret['unionid']);
				$dbWechatUserinfo->update();
				return $openid;
			}
		}
		return false;
