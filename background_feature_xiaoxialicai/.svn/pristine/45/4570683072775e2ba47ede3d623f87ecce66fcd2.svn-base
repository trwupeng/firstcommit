			...
			try{
				...
				if(\Sooh\DB\Broker::errorIs($e,\Sooh\DB\Error::tableNotExists)){
					try{
						$db->ensureObj($tb, [
							'deviceId'=>'varchar(64)',
							'deviceIdAlias'=>'varchar(64)',
							'dtChange'=>'bigint',
							'userIdOld'=>'bigint',
							'extraDataOld'=>'varchar(512)',
							'extraRetOld'=>'varchar(512)',
							'contractIdOld'=>'bigint',
							'userIdNew'=>'bigint',
							'extraDataNew'=>'varchar(512)',
							'extraRetNew'=>'varchar(512)',
							'contractIdNew'=>'bigint',
							'ipOld'=>'varchar(16)',
							'ipNew'=>'varchar(16)',
						]);
						$db->addRecord($tb, $chg);
					} catch (\Exception $e) {
						error_log("ERROR on write device log:".$e->getMessage()."\n".$e->getTraceAsString());
					}
				}else{
					error_log("ERROR on write device log:".$e->getMessage()."\n".$e->getTraceAsString());
				}
