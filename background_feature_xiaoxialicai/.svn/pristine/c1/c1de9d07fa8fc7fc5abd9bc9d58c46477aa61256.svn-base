                ...
                if($msgType == 2) {
                    foreach($exsitsUserPhones as $phone) {
                        ...
                        if(is_array($rs)) {
                            ...
                            $user_obj->update();
                            $bonusItemClass =  '\\Prj\\Items\\'.$fields['voucherName'];
                            $voucherItem = new $bonusItemClass(['userId'=>$record['userId']]);
                            $voucherItem->sendMsg(); // 站内信

                            $record['flgVoucher']   = 1;
                            $r = $this->sendMsg($phone, $fields['msg']); // 通知短信
                            if ($r) {
                    ...
                    foreach($exsitsUserPhones as $k=> $phone) {
                        ...
                        if(is_array($rs)) {
                            ...
                            // 发站内信
                            $bonusItemClass =  '\\Prj\\Items\\'.$fields['voucherName'];
                            $voucherItem = new $bonusItemClass(['userId'=>$users[$phone]['userId']]);
                            $voucherItem->sendMsg(); // 站内信
