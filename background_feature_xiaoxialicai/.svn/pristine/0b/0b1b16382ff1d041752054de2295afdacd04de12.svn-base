...
//            $cal->setField('realTotalWithdraw', $cal->getField('realTotalWithdraw') + $inputs['amount']);
            ...
            if($ret['code']==400){
                ...
                //推送
                $this->loger->resChanged = array_merge($this->loger->redChanged, [
                    'action_ask_money' => [
...
//            $smsReg->sendCode(\Prj\SMS::$formats['resetTradePwd'], $smsCode);
        ...
        if(in_array($cmd,['first','second'])){
            ...
            try{
                if($cmd == 'first'){
                    ...
                    //todo 多设备踢人...
                    $pushContent = json_encode(['type' => 'kickout', 'msg' => '您的银行卡绑定信息已更改，请重新登录！']);
                    $this->loger->sarg1 = json_encode(['all', $userId, null, $pushContent]);
//                    try {
//                        \Lib\Services\Push::getInstance()->push('all', $userId, null, $pushContent);//旧版本
