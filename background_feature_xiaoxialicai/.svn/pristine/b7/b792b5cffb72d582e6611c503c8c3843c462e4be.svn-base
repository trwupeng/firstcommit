...
//         );
    ...
    public function indexAction($type = '')
    {
        ...
        $frm->addItem('_waresname_lk', form_def::factory('标的名称', '', form_def::text))
            ->addItem('_waresId_eq', form_def::factory('标的ID', '', form_def::text))
            ...
            ->addItem('_timeStartReal_g2', form_def::factory('实际上架起始日期', '', form_def::datepicker, []))
            ->addItem('_timeStartReal_l2', form_def::factory('实际上架截止日期', '', form_def::datepicker, []))
        ...
        $frm->fillValues();
        if($this->_request->get('_timeStartReal_g2'))$frm->resetValue('_timeStartReal_g2',date('Ymd000000',strtotime($this->_request->get('_timeStartReal_g2'))));
        if($this->_request->get('_timeStartReal_l2'))$frm->resetValue('_timeStartReal_l2',date('Ymd235959',strtotime($this->_request->get('_timeStartReal_l2'))));
        if ($frm->flgIsThisForm) { //submit
            $where = $frm->getWhere();
        ...
        if (!$ware->exists()) {
            ...
            if (empty($investList)) {
                ...
                try {
                    ...
                    $ret = \Lib\Services\PayGWCmd::sendToPayGWCmd('trade_unfreeze',[$waresId,'']);
                } catch (\Sooh\Base\ErrException $e) {
