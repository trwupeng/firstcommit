...
class CheckController extends \Prj\ManagerCtrl {
    ...
    public function dayCompanyAction(){
        ...
        $frm->addItem('_sn_eq', form_def::factory('流水号', '', form_def::text))
            ...
            ->addItem('_payYmd_g2', form_def::factory('起始日期', '', form_def::datepicker))
            ->addItem('_payYmd_l2', form_def::factory('截止日期', '', form_def::datepicker))
            ->addItem('pageId', $pageid)
            ->addItem('pageSize', $this->pager->page_size);
        $frm->fillValues();
        if ($frm->flgIsThisForm) {
            $where = $frm->getWhere();
            var_log($where,'where >>> ');
            if($where['payYmd]'])$where['payYmd]'] = date('Ymd000000',strtotime($where['payYmd]']));
            if($where['payYmd['])$where['payYmd['] = date('Ymd235959',strtotime($where['payYmd[']));
        } else {
            $where = [];
        }
        //合并表单的查询条件
        ...
        $rs = $newArr;
        if($isDownloadEXCEL){
            array_walk($rs,function(&$v,$k){
                $v['sn'] = '_'.$v['sn'];
                $v['userId'] = $v['userId']?'_'.$v['userId']:'';
                $v['waresId'] = $v['waresId']?'_'.$v['waresId']:'';
                $v['rebateId'] = $v['rebateId']?'_'.$v['rebateId']:'';
            });
            return $this->downEXCEL($rs, array_keys($header),null,true);
        }
        //输出
