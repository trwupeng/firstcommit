...
class Message
{
     ...
     */
    const DEFAULT_MSG_CONF_PATH = '/application/library/Prj/Message/Conf/message.php';
            ...
            if ($v['event'] == $args['event']) {
                if (preg_match_all('(\{\w+\})', $v['value'], $match) >= 0) {
                    ...
                    foreach ($match[0] as $matchV) {
                        ...
                        if (isset($args[$replaceV])) {
                            $arrReplace[] = $args[$replaceV];
                        } else {
                            //加入固定参数
                            switch ($replaceV) {
                                case 'brand':
                                    $arrReplace[] = \Prj\Message\Message::MSG_BRAND;
                                    break;
                                default:
                                    $arrReplace[] = '__';
                                    break;
    ...
    private static function readConf($fileName = '')
    {
        if (empty(self::$msgConf)) {
            $file = $fileName ? : APP_PATH . self::DEFAULT_MSG_CONF_PATH;
    ...
    public static function send($string, $value, $params)
    {
        ...
        if ($value['msg'] == 1) {
            try {
                ...
                if (is_numeric($_msgType) && $_msgType >= 1) {
                    //other
                } else {
                    $_msgType = 7;
                }
