...
class UserController extends \Prj\UserCtrl
{
    ...
    protected function _bindCard($inputs, $sys)
    {
        ...
        try {
            $ret = $sys->binding($sn, $userId, $inputs['realName'], $inputs['idCardType'], $inputs['idCardSn'], $inputs['bankId'], $inputs['bankCard'], $inputs['phone'], \Sooh\Base\Tools::remoteIP());
...
//        }
    ...
    {
//	return $this->returnError('系统升级中，请稍后再试');
       ...
       // \Prj\Tool\Func::debug_log();
        ...
        if ($ordersOfCharge) {
            ...
            try {
                ...
                if(!empty($e->customData))$ret = $e->customData;
            }
            if($ret['code']==400){
    ...
    protected function _rechargeCode($inputs, \Lib\Services\PayGW $sys)
    {
        ...
        error_log('user->_rechargeCode:'.$ret['code']);
        if($ret['code']==400){
            return $ret['msg']?$ret['msg']:'rpc_failed:rechargeCode';
        }else{
     ...
     * @errors {code:400,msg:'error_paypwd'} 错误的支付密码
    ...
    */
    ...
    public function checkPaypwdAction()
    {
        ...
        if (is_array($checkPayPwd)) {
            ...
            return $this->returnError(\Prj\Lang\Broker::getMsg('system.error_custom'));
        } else {
...
//	    $ret = \Prj\Data\Vouchers::loopGetRecordsPage(array('timeCreate' => 'rsort', 'voucherId' => 'sort'), ['where' => $map], $pager);
    ...
    public function sendSmsCodeAction()
    {
        ...
        try {
            ...
            if (\Sooh\Base\Tests\Bomb::blowup('User_SendSmsCode_Return_Code', false)) {
                $this->_view->assign('data', ['smsCode' => $smsCode]);//出现多个assign中的data字段重复，修改如下突出唯一识别
                $this->_view->assign('datasendsmscode', ['smsCode' => $smsCode]);
                return $this->returnOK(['bomb' => $this->_request->get('__testBomb__')]);
            }

            $smsReg = \Sooh\DB\Cases\SMSCode::getCopy($phone);
            $smsReg->sendCode(\Prj\SMS::$formats['register'], $smsCode);
            \Lib\Services\SMS::getInstance()->sendCode($phone, $smsCode, \Prj\SMS::$formats['resetTradePwd']);
    ...
    */
    ...
    public function setToPushAction()
    {
	    ...
	    $setsArr = array_values(\Prj\ReadConf::$pushType);
...
//        $setsArr = ['all', 'bid', 'contractIssued', 'repayment', 'withdrawal', 'redPacket', 'rebate'];
    ...
    public function getPushSettingAction()
    {
        ...
        try {
            ...
            if ($dbUser->exists()) {
	            ...
	            $typeMapName = \Prj\ReadConf::getPushMap();
	            $list = [];
    ...
    public function rechargeDetailAction()
    {
        ...
        $delete = ['amountAbs', 'poundage', 'batchId', 'withdrawYmd', 'iRecordVerID'];
        if (!empty($delete)) {
            foreach ($delete as $v) {
                unset($data[$v]);
            }
...
//		$this->_view->assign('uri', \Sooh\Base\Ini::getInstance()->get('uriBase')['www'] . '/index.php?__=user/getInviteList&__VIEW__=json');
     ...
     * @output {"code":200,"msg":"success","data":[{"nickname":"nickname","counts":"counts","zhName":"zhName","amount":"amount","ymd":"ymd","his":"his"},{"nickname":"..."}]}
    ...
    public function getInviteListAction()
    {
	    ...
	    $pager = new \Sooh\DB\Pager($pageSize);
	    $pager->init($counts, $pageId);
	    $list = \Prj\Data\Rebate::paged($pager, ['userId' => $userId, 'statusCode!' => -1], 'rsort createYmd', $userId);
...
//            $ret = \Prj\Data\Vouchers::loopGetRecordsPage(['timeCreate' => 'rsort', 'voucherId' => 'sort'], $where, $pager);
            ...
            if ($pageId == 1) {
                ...
                $usedNumForParent = empty($keys) ? 0 : \Prj\Data\VouchersInterim::loopGetRecordsCount(['pid' => $keys, 'isUsed' => 2]);//被使用
