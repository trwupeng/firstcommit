...
/////////////////////////////////////////////////////////TODO
        ...
        if (!$ymdFirstBought) { //首购松泉 //debug  //
            ...
            if (!empty($newVoucherId)) {
                try {
                    \Prj\ReadConf::run(
                            ['event' => 'red_invite_packet', 'num_packet' => 1, 'private_gift' => $vouchersGiftArr['amount'] / 100, 'num_deadline' => 48, 'brand' => \Prj\Message\Message::MSG_BRAND], ['userId' => $this->user->userId, 'phone' => $this->user->getField('phone')]
                    );
                } catch (\ErrorException $e) {
        ...
        try {
            $ret = \Prj\ReadConf::run(
                            ['event' => 'suc_all', 'pro_name' => $wares->getField('waresName'),], ['phone' => $phone, 'userId' => $this->user->userId]
            );
            $ret1 = \Prj\ReadConf::run(
                            ['event' => 'join_us', 'brand' => \Prj\Message\Message::MSG_BRAND, 'cont_ok' => '订单详情', 'time_all' => 24], ['phone' => $phone, 'userId' => $this->user->userId]
            );
        ...
        if (empty($parUserId)) {
            ...
            try {
                ...
                if (!empty($weekActiveBonus)) {
                    \Lib\Services\Push::getInstance()->push('all', $parUserId, null, json_encode($weekActiveBonus));
                }
                ...
                if (empty($pid)) {
                    ...
                    if (!$parVoucher->exists()) {
                        ...
                        try {
                            ...
                            $user->update();
                            if (!empty($weekActiveBonus)) {
                                \Lib\Services\Push::getInstance()->push('all', $userId, null, json_encode($weekActiveBonus));
                            }
                            $ret[$userId] = 1;
                        } catch (\ErrorException $e) {
