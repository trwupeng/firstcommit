...
//				$v = $child->options['_CtrlAct_'];
		...
		if(!empty($records)){
			...
			foreach($records as $rowid=>$r){
				$new = array($r['loginName'],$r['nickname'], $this->depts[$r['dept']], \Prj\Misc\View::fmtYmd($r['lastYmd']),$r['lastIP'],);

				if ($r['rights'] == '*.*') {
					$tmp = '尚未设置权限，默认拥有所有的权限';
...
//					unset($fields['rights']);
		...
		if($frm->flgIsThisForm){//submit
			try{
				...
				if($ret===true){
                    ...
                    $this->returnError(\Prj\Lang\Broker::getMsg('managers.passwd_rset_fail'));
				}else{
			...
			if(!empty($where)){
			    ...
			    $this->returnError(\Prj\Lang\Broker::getMsg('managers.unknown_manager'));
			}
		}
	}
	...
	public function disableAction()
	{
		...
		if($ret){
			...
			$this->returnError(\Prj\Lang\Broker::getMsg('managers.forbidden_fail'));
